...
const setMPA = () => {
    ...
    const entryFiles = glob.sync(path.resolve(__dirname, './src/modules/*/index-search.js'));
    Object.keys(entryFiles).map( i => {
        const entryFile = entryFiles[i];

        // 获取html文件名
        const match = entryFile.match(/src\/modules\/(.*)\/index-search\.js/);
...
module.exports = {
    ...
    output: {
        ...
        filename: 'chanks/[name]-server.js',
        libraryTarget: "umd" 
    },
